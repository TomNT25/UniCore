...
namespace UniCore.Application.Feature.v1.Auth.Login
{
    public class LoginHandler : IRequestHandler<LoginRequestDTO, LoginResponseDTO>
    {
        ...
        public async Task<LoginResponseDTO> HandleAsync(LoginRequestDTO request, CancellationToken cancellationToken)
        {
            ...
            if (userEntity == null && request.Username.Contains('@'))
            {
                var userByEmail = await _userRepository.GetByEmailAsync(request.Username, cancellationToken);
                if (userByEmail != null)
                {
                    userEntity = _mapper.Map<UserDTO>(userByEmail);
                }
            }
            ...
            var mfaSetting = await _userMfaSettingRepository.GetByUserIdAsync(userEntity.Id, cancellationToken);
            bool isMfaEnabled = mfaSetting != null && mfaSetting.IsMfaEnabled && mfaSetting.IsActive;
