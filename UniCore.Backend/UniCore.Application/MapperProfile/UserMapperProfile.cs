    ...
    public class UserMapperProfile : IRegister
    {
        public void Register(TypeAdapterConfig config)
        {
            ...
            config.NewConfig<User, UserDetailDTO>()
                .Map(dest => dest.StudentCode, src => src.StudentCode ?? src.Code ?? string.Empty)
                .Map(dest => dest.ClassName, src => src.Class != null ? src.Class.Name : null)
                .Map(dest => dest.Roles, src => src.UserRoles != null
                    ? src.UserRoles.Where(ur => ur.Role != null).Select(ur => ur.Role.Name).ToList()
                    : new List<string>())
                .Map(dest => dest.Status, src => src.IsActive ? "active" : "inactive")
                .Map(dest => dest.Provider, src => src.Provider != null ? src.Provider.ToUpperInvariant() : "SYSTEM")
                .Map(dest => dest.Profile, src => src.UserProfile != null ? new UserDetailProfileDTO
                {
                    FirstName = src.UserProfile.FirstName,
                    LastName = src.UserProfile.LastName,
                    FullName = src.UserProfile.FullName,
                    PhoneNumber = src.UserProfile.PhoneNumber,
                    AvatarUrl = src.UserProfile.AvatarUrl,
                    CidFrontImageUrl = src.UserProfile.CidFrontImageUrl,
                    CidBackImageUrl = src.UserProfile.CidBackImageUrl,
                    Gender = src.UserProfile.Gender,
                    BirthDate = src.UserProfile.BirthDate,
                    Address = src.UserProfile.Address,
                    IsActive = src.UserProfile.IsActive,
                    IsVerified = src.UserProfile.IsVerified,
                    VerifiedAt = src.UserProfile.VerifiedAt
                } : null);
